...
import { sliceRelation, Operations, isFieldExpression, getFieldExpressionRef } from './utils';
import { createRelationExpression } from './ExpressionBuilder';
import { iterateLogicalExpression, getPropertiesFromExpression } from './LogicalIterator';
...
const nullToZero = function (tableAlias, columnAlias = 'count') {
    ...
    return raw('case when ?? is null then 0 else cast(?? as decimal) end as ??', [column, column, columnAlias]);
};
